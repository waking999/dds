...
import au.edu.cdu.problems.ds2.GreedyVote;
 ...
 * <p>
 * The difference between dds and dds3 is that, in dds3, we add
 * 1. greedynaive in iteration
 *
 * @author kwang
 */
public class GreedyVoteL2HDDS3 implements IAlgorithm {
    ...
    @Override
    public void compute() {
        GlobalVariable voteG = AlgoUtil.copyAndAdjustGlobalVariable(g);
                ...
                if (!idxDomed[vIdx]) {
                    ...
                    if (uIdx != ConstantValue.IMPOSSIBLE_VALUE) {
                        ...
                        if (isMomentOfRegret(p, giStepV)) {
                            ...
                            //copy the solution for future comparasion
                            int giSIdexSolSizeCopy = giS.getIdxSolSize();
                            //int[] gisIdxSol= giS.getIdxSol();
                            //int[] gisIdxSolCopy=Arrays.copyOf(gisIdxSol,gisIdxSol.length);
                            ...
                            GlobalVariable giSCopy = AlgoUtil.copyAndAdjustGlobalVariable(giS);
                            int giSCopyIdxSolSize;
                            //apply greedyvote
                            IAlgorithm algo = new GreedyNaive();
                            algo.setGlobalVariable(giSCopy);
                            algo.compute();
                            giSCopyIdxSolSize = giSCopy.getIdxSolSize();

                            r = Math.min(r, giIdxSolSize - giSCopyIdxSolSize - 1);
                            ...
                            for (int tryR = 1; tryR <= r; tryR++) {
                                ...
                                if (validCombin != null) {
                                    ...
                                    //giSIdexSolSizeCopy: d1+d2
                                    //giSCopyIdxSolSize: greedyvote solution
                                    //giIdxSolSize: ddsfpt solution
                                    if ((giIdxSolSize < giSIdexSolSizeCopy) && (giIdxSolSize < giSCopyIdxSolSize)) {
    ...
    private static int getIdxSolSize(int[] idxSol, int idxSolSize, int[] d2Rm) {
        for (int vIdxRm : d2Rm) {
