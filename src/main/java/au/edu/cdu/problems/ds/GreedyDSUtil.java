...
//
//		sb.append(ConstantValue.COMMA).append(dominatingSet.size()).append(ConstantValue.COMMA).append(k)
//				.append(ConstantValue.COMMA).append(r).append(ConstantValue.COMMA);
//
...
//			sb.append(runningTime).append(ConstantValue.COMMA).append(desc).append(ConstantValue.COMMA);
//		}
//
...
//	}
	...
	public static <V, E> List<V> getUAndOneNeighborToBeDominated(Graph<V, E> g, Map<V, Boolean> dominatedMap,
		...
		Util.addElementToList(uList, u);
		Util.addElementToList(uList, v);
		Collection<V> uNeig = g.getNeighbors(u);
		uNeig.removeAll(gIPre.getVertices());

		List<V> oneNeighborList =  Util.getFirstNItemsInCollection(1, uNeig);
...
//
//		long end = System.nanoTime();
//		runningTimeMap.put(ConstantValue.RUNNING_TIME_DEGREERR, (end - start));
//
...
//			Integer u = AlgoUtil.getHighestUtilityNeighborOfAVertex(v, g, dominatedMap);
//
//			dI = new ArrayList<Integer>();
//			 Util.addElementToList(dI, u);
//
...
//
//			/*
//			 * the difference between the current solution and the solution at
//			 * back up point could be a considerable solution in dds fpt
//			 */
...
//		}
//		long end = System.nanoTime();
//		Long existingRunningTime = runningTimeMap.get(ConstantValue.RUNNING_TIME_DDS);
//		if (existingRunningTime == null) {
//			existingRunningTime = Long.valueOf(0);
//		}
//		runningTimeMap.put(ConstantValue.RUNNING_TIME_DDS, existingRunningTime + (end - start));
//		return ddsI;
//	}
	...
	public static <V, E> void applyMinimal(Graph<V, E> g, List<V> dominatingSet, Map<String, Long> runningTimeMap)
		...
		long start = System.nanoTime();
		dominatingSet = AlgoUtil.minimal(g, dominatingSet);
		long end = System.nanoTime();

		Long existingRunningTime = runningTimeMap.get(ConstantValue.RUNNING_TIME_MINI);
		if (existingRunningTime == null) {
			existingRunningTime = Long.valueOf(0);
		}
		runningTimeMap.put(ConstantValue.RUNNING_TIME_MINI, existingRunningTime + (end - start));
	}
	...
	public static <V, E> void applyLS(Graph<V, E> g, List<V> dominatingSet, Map<String, Long> runningTimeMap) {
		...
		long start = System.nanoTime();
		AlgoUtil.grasp(g, dominatingSet);
		long end = System.nanoTime();

		Long existingRunningTime = runningTimeMap.get(ConstantValue.RUNNING_TIME_LS);
		if (existingRunningTime == null) {
			existingRunningTime = Long.valueOf(0);
		}
		runningTimeMap.put(ConstantValue.RUNNING_TIME_LS, existingRunningTime + (end - start));
	}
	...
	public static List<Integer> useGreedyToCalcDS(Graph<Integer, String> g, Map<String, Long> runningTimeMap)
		...
		long end = System.nanoTime();
		Long existingRunningTime = runningTimeMap.get(ConstantValue.RUNNING_TIME_GUARANTEE);
		if (existingRunningTime == null) {
			existingRunningTime = Long.valueOf(0);
		}
		runningTimeMap.put(ConstantValue.RUNNING_TIME_GUARANTEE, existingRunningTime + (end - start));
	...
	public static void initRunningTimeMap(Map<String, Long> runningTimeMap) {
		...
		runningTimeMap.put(ConstantValue.RUNNING_TIME_POLYRR, Long.valueOf(0));
		runningTimeMap.put(ConstantValue.RUNNING_TIME_DEGREERR, Long.valueOf(0));
		runningTimeMap.put(ConstantValue.RUNNING_TIME_DDS, Long.valueOf(0));
		runningTimeMap.put(ConstantValue.RUNNING_TIME_GUARANTEE, Long.valueOf(0));
		runningTimeMap.put(ConstantValue.RUNNING_TIME_MINI, Long.valueOf(0));
		runningTimeMap.put(ConstantValue.RUNNING_TIME_LS, Long.valueOf(0));
		runningTimeMap.put(ConstantValue.RUNNING_TIME_TOTAL, Long.valueOf(0));
	}
	...
	public static <V> List<V> getUndominatedVertices(Map<V, Boolean> dominatedMap) {
		...
		for (V key : keySet) {
			if (!dominatedMap.get(key)) {
				 Util.addElementToList(rtnList, key);
			}
		}
...
//				 Util.addElementToList(verticesToAddInGraph, w);
//			}
//		}
//		 Util.addElementToList(verticesToAddInGraph, u);
//
//		AlgoUtil.prepareGraph(gRef, gI, verticesToAddInGraph);
//	}
